...
const MOCK_LEADERBOARD: LeaderboardUser[] = [
  { rank: 1, username: "Sarah_J", level: 45, xp: 12500, avatarColor: "from-yellow-400 to-orange-500" },
  { rank: 2, username: "MaxPower", level: 42, xp: 11200, avatarColor: "from-slate-300 to-slate-500" },
  { rank: 3, username: "Elena_V", level: 40, xp: 10850, avatarColor: "from-amber-600 to-orange-700" },
  { rank: 4, username: "David_Chen", level: 38, xp: 9500, avatarColor: "from-primary-sky to-primary-teal" },
  { rank: 5, username: "Zoe_A", level: 35, xp: 8200, avatarColor: "from-accent-purple to-pink-500" },
  { rank: 6, username: "Marcus_W", level: 34, xp: 7900, avatarColor: "from-emerald-400 to-teal-500" },
  { rank: 7, username: "Chloe_S", level: 31, xp: 7100, avatarColor: "from-rose-400 to-red-500" },
];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-6">
      ...
      <div className="flex justify-center items-end gap-4 md:gap-8 h-64 mb-12 mt-8">
        ...
        >
          <div className={`w-20 h-20 rounded-full border-4 border-white shadow-lg bg-gradient-to-br ${MOCK_LEADERBOARD[1].avatarColor}`} />
          ...
          <div className="absolute -bottom-10 text-center w-full">
            <p className="font-bold text-slate-700 truncate">{MOCK_LEADERBOARD[1].username}</p>
            <p className="text-xs text-primary-teal font-medium">{MOCK_LEADERBOARD[1].xp} XP</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className={`w-24 h-24 rounded-full border-4 border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.5)] bg-gradient-to-br ${MOCK_LEADERBOARD[0].avatarColor}`} />
          ...
          <div className="absolute -bottom-10 text-center w-full">
            <p className="font-bold text-slate-800 truncate text-lg">{MOCK_LEADERBOARD[0].username}</p>
            <p className="text-sm text-primary-sky font-bold">{MOCK_LEADERBOARD[0].xp} XP</p>
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          className="flex flex-col items-center gap-3 relative"
        >
          <div className={`w-20 h-20 rounded-full border-4 border-white shadow-lg bg-gradient-to-br ${MOCK_LEADERBOARD[2].avatarColor}`} />
          ...
          <div className="absolute -bottom-10 text-center w-full">
            <p className="font-bold text-slate-700 truncate">{MOCK_LEADERBOARD[2].username}</p>
            <p className="text-xs text-primary-teal font-medium">{MOCK_LEADERBOARD[2].xp} XP</p>
          </div>
        </motion.div>
      </div>
          ...
          {MOCK_LEADERBOARD.map((user, index) => (
            ...
            >
              ...
              <div className="col-span-6 md:col-span-5 flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full border-2 border-white shadow-sm bg-gradient-to-br ${user.avatarColor}`} />
                <span className="font-bold text-slate-700">{user.username}</span>
              </div>
              
              <div className="col-span-2 hidden md:flex justify-center">
                <span className="font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full text-sm">
                  {user.level}
                </span>
              </div>
